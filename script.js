...
// Incluye: Lógica del Chat con IA, Registro en Make.com y Lógica del Formulario.
document.addEventListener('DOMContentLoaded', function() {
    ...
    // CORRECCIÓN: Eliminado el punto final del nombre del modelo.
    ...
    async function handleSendMessage() {
        ...
        // CORRECCIÓN: Se eliminó la comprobación de API key defectuosa.
    ...
    async function logDataToMake(data, fullConversationHistory) {
        ...
        try {
            const chatText = fullConversationHistory
                .filter(msg => !(msg.role === 'user' && msg.parts[0].text.includes('REGLAS ESTRICTAS DEL SISTEMA')))
                .map(msg => {
    ...
    if (assistantButtonChat) {
        assistantButtonChat.addEventListener('click', () => {
            ...
            if (chatMessages && chatMessages.children.length === 0) {
                 // CORRECCIÓN: Mensaje de bienvenida simple y robusto para evitar errores.
