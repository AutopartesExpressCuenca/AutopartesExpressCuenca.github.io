...
// Reconstruido desde la última versión funcional para garantizar estabilidad.
document.addEventListener('DOMContentLoaded', function() {
    ...
    // ¡¡¡ ATENCI se trata como un mensaje normal
    ...
    // MODIFICACIÓN QUIRÚRGICA: Esta función ahora construye el texto del chat
    async function logDataToMake(data, fullConversationHistory) {
        ...
        try {
            // Construye el texto completo del chat a partir del historial
            const chatText = fullConversationHistory
                .filter(msg => !(msg.role === 'user' && msg.parts[0].text.includes('REGLAS ESTRICTAS DEL SISTEMA'))) // Filtra el prompt del sistema
                .map(msg => {
            ...
            const fullData = { 
                ...data, 
                texto_chat_completo: chatText, // Añade el nuevo campo con el chat formateado
            ...
            await fetch(makeWebhookLoggerUrl, {
                method: 'POST',
                // Se eliminó 'mode: no-cors' para que los datos lleguen separados
    ...
    if (assistantButtonChat) {
        assistantButtonChat.addEventListener('click', () => {
            ...
            // Si el chat está vacío, muestra el primer mensaje del asistente
            if (chatMessages && chatMessages.children.length === 0) {
                 const initialBotMessage = conversationHistory.find(m => m.role === 'model')?.parts[0]?.text;
                 if (initialBotMessage) {
                    addMessage('assistant', initialBotMessage);
                 }
            }
            addChatListeners();
            if(chatInput) chatInput.focus();
    ...
    // --- ESTADO DE LA CONVERSACIÓN (PROMPT ENFOCADO Y PROBADO) ---
            ...
            try {
                ...
                try {
                    ...
                    if (jsonMatch) {
                        ...
                        if (responseObject.accion === 'registrar_cotizacionanio');
    ...
    const marcasFullList = { "Chevrolet": ["Onix", "Onix RS", "Onix Turbo Sedán", "Joy HB", "Joy Sedán", "Aveo", "Spark GT", "Spark Life", "Beat", "Sail", "Cavalier", "Cruze", "Bolt", "Bolt-EUV", "Groove", "Tracker", "Captiva", "Captiva XL", "Equinox-EV", "Blazer-RS-EV", "Tahoe", "Trailblazer", "Montana", "D-Max (varias gen.)", "Colorado", "Silverado", "Blazer (hist.)", "Trooper", "LUV", "Luv-D-Max", "Rodeo", "Gemini", "Corsa", "Esteem", "Forsa", "Vitara (3 puertas)", "Vitara (5 puertas)", "Grand Vitara", "Blue-Bird", "chasis MR-buses"], "Kia": ["Picanto", "Rio", "Rio-5", "Soluto", "Cerato", "K3", "Carens", "Carnival", "Stonic", "Stonic Hybrid", "Seltos", "Sonet", "Sportage", "Sorento", "Niro", "Niro-EV", "EV6", "EV5", "EV9", "Soul-EV"], "Toyota": ["Agya", "Yaris", "Yaris Sport", "Yaris Cross", "Corolla", "Corolla Híbrido", "Corolla Cross Híbrido", "C-HR", "Raize", "RAV4", "Rush", "Prius", "Prius-C", "Innova", "Hilux", "Tacoma", "Fortuner", "Land Cruiser Prado", "Land Cruiser 200", "Land Cruiser 300", "4Runner", "FJ Cruiser", "Starlet", "Tercel", "Celica"], "Hyundai": ["Accent", "Grand i10", "Elantra", "Sonata", "Venue", "Kona", "Kona Hybrid", "Tucson", "Santa Fe", "Creta", "Staria"], "Chery": ["QQ3", "QQ6", "Nice-A1", "Van-Pass", "XCross", "Arrizo-3", "Arrizo-5", "Tiggo", "Tiggo-2", "Tiggo-2 Pro", "Tiggo-3", "Tiggo-4", "Tiggo-5", "Tiggo-7", "Tiggo-7 Pro", "Tiggo-8", "Tiggo-8 Pro"], "Suzuki": ["Swift", "Baleno", "Celerio", "Ignis", "Vitara", "Grand Vitara", "Jimny", "XL7", "Ertiga", "S-Cross", "SX4"], "Renault": ["Kwid", "Sandero", "Logan", "Stepway", "Duster", "Captur", "Koleos", "Oroch", "Kangoo", "Symbol", "Megane", "Fluence"], "Great Wall": ["Wingle-1", "Wingle-2", "Wingle-3", "Poer", "Haval H2", "Haval H6", "Haval H9", "Haval Jolion", "Haval F7", "M4", "ORA Good-Cat", "Tank-300"], "JAC": ["J2", "J4", "J5", "S2", "S3", "S5", "S7", "T40", "T60", "V7", "HFC-1037"], "DFSK": ["Glory-500", "Glory-560", "Glory-580", "F5", "Mini Truck", "C31", "C52", "EC35", "K05", "K07"], "Volkswagen": ["Gol", "Escarabajo (Tipo-1)", "Voyage", "Polo", "Virtus", "T-Cross", "Tiguan", "Taigo", "Jetta", "Passat", "Amarok"], "Nissan": ["March", "Versa", "Sentra", "Kicks", "X-Trail", "Frontier", "NV350' && responseObject.datos) {
                            ...
                            // Llamada a la función de registro, pasándole los datos y el historial completo.
                            await logDataToMake(responseObject.datos, conversationHistory); 
                            return; 
                        }
                    }
                } catch (e) {
    ...
    // FUNCIÓN MEJORADA: Construye el historial del chat y lo envía a Make.com
    async function logDataToMake(data, fullConversationHistory) {
        if (!makeWebhookLoggerUrl || makeWebhookLoggerUrl === 'PEGA_AQUI_TU_NUEVA_URL_DE_WEBHOOK_DE_MAKE') {
            console.error("URL del webhook de registro de Make.com no configurada.");
            return;
        ...
        try {
            // Formatea el historial del chat en un solo string
            const chatText = fullConversationHistory
                .filter(msg => !(msg.role === 'user' && msg.parts[0].text.includes('REGLAS ESTRICTAS DEL SISTEMA'))) // Filtra el prompt del sistema
                .map(msg => {
                    const prefix = msg.role === 'user' ? 'Cliente:' : 'Alex:';
            ...
            const fullData = { 
                ...data, 
                texto_chat_completo: chatText, // Añade el chat formateado a los datos
                fecha: now.toLocaleDateString('es-EC', { timeZone: 'America/Guayaquil' }), 
                hora: now.toLocaleTimeString('es-EC', { timeZone: 'America/Guayaquil' }) 
            };
    ...
    // LÓGICA DEL BOTÓN ASISTENTE (SIMPLIFICADA Y ROBUSTA)
    if (assistantButtonChat) {
        assistantButtonChat.addEventListener('click', () => {
            if (!chatWidget) return;
            chatWidget.classList.remove('hidden');
            if (chatMessages && chatMessages.children.length === 0) {
                 // Mensaje de bienvenida simple. No depende de nada más para evitar fallos.
    ...
    const updateFormSteps = () => { if (!formSteps || !progressSteps || !form) return; formSteps.forEach((s, i) => s.classList.toggle('active', i === currentStep)); progressSteps.forEach((s, i)getElementById('anio');
    const logosContainer = document.getElementById('logos-container');
    const otroMarcaContainer = document.getElementById('otra-marca-container');
    const otroModeloContainer = document.getElementById('otro-modelo-container');
    const otroAnioContainer = document.getElementById('otro-anio-container');
    ...
    let popupTimeout;
    const marcasPopulares = ["Chevrolet", "Kia", "Toyota", "Hyundai", "Suzuki", "Renault", "Great Wall", "Mazda", "Nissan", "Ford", "Volkswagen", "Mitsubishi"];
    const marcasFullList = { "Chevrolet": ["Onix", "Onix RS", "Onix Turbo Sedán", "Joy HB", "Joy Sedán", "Aveo", "Spark GT", "Spark Life", "Beat", "Sail", "Cavalier", "Cruze", "Bolt", "Bolt-EUV", "Groove", "Tracker", "Captiva", "Captiva XL", "Equinox-EV", "Blazer-RS-EV", "Tahoe", "Trailblazer", "Montana", "D-Max (varias gen.)", "Colorado", "Silverado", "Blazer (hist.)", "Trooper", "LUV", "Luv-D-Max", "Rodeo", "Gemini", "Corsa", "Esteem", "Forsa", "Vitara (3 puertas)", "Vitara (5 puertas)", "Grand Vitara", "Blue-Bird", "chasis MR-buses"], "Kia": ["Picanto", "Rio", "Rio-5", "Soluto", "Cerato", "K3", "Carens", "Carnival", "Stonic", "Stonic Hybrid", "Seltos", "Sonet", "Sportage", "Sorento", "Niro", "Niro-EV", "EV6", "EV5", "EV9", "Soul-EV"], "Toyota": ["Agya", "Yaris", "Yaris Sport", "Yaris Cross", "Corolla", "Corolla Híbrido", "Corolla Cross Híbrido", "C-HR", "Raize", "RAV4", "Rush", "Prius", "Prius-C", "Innova", "Hilux", "Tacoma", "Fortuner", => s.classList.toggle('active', i <= currentStep)); form.scrollIntoView({ behavior: 'smooth', block: 'start' }); };
 ...
 "Taigo", "Jetta", "Passat", "Amarok"], "Nissan": ["March", "Versa", "Sentra", "Kicks", "X-Trail", "Frontier", "NV350", "Pathfinder", "Note", "Micra"], "Mazda": ["Mazda2", "Mazda3", "Mazda6", "CX-3", "CX-30", "CX-5", "CX-9", "CX-50", "CX-90", "BT-50"], "Dongfeng": ["Rich-6", "Rich-7", "Rich-12", "S30", "Husky", "EQ2030", "EQ2050", "580", "580 Pro", "mini-van Q30"], "Sinotruk": ["Howo-7", "Howo-9", "A7", "G7", "T5G", "ZZ1257", "ZZ1325", "ZZ1507", "ZZ3317", "ZZ4251"], "Jetour": ["X70", "X90", "X95", "T1", "T5", "T8", "Dasheng", "Cruiser", "XC", "Cooler"], "Ford": ["Fiesta", "EcoSport", "Ranger", "Explorer", "Mustang", "Transit", "Everest", "Bronco", "F        logosContainer.appendChild(otroWrapper);
        otroWrapper.onclick = () => {
            document.querySelectorAll('.logo-wrapper.selected').forEach(w => w.classList.remove('selected'));
    ...
    const brands-150", "Edge"], "Changan": ["CS35", "CS55", "CS75", "CS85", "Alsvin", "UNI-T", "Eado", "Eado Xt", "Benni", "CS15"], "BYD": ["Atto-3", "Dolphin", "Seal", "Song-Plus", "Tang", "Yuan-EV", "Qin", "e1", "e2", "Han"], "Subaru": ["Impreza", "XV", "Forester", "Outback", "WRX", "Crosstrek", "Legacy", "BRZ", "Solterra", "Ascent"], "BannerTrack = document.getElementById('brands-banner-track');
    ...
    if(Citroen": ["C3", "C3 Aircross", "C4", "C5 Aircross", "Berlingo", "C-Elysée", "C4 Cactus", "SpacetourerprevBtns) prevBtns.forEach(btn => btn.addEventListener('click', () => { currentStep--; updateFormSteps(); }));
    if(modeloSelect) modeloSelect.addEventListener('change', () => { if (modeloSelect.value === "Otro") { otroModeloContainer.style.display = 'block'; otroModeloInput.required = true; updateLiveData('modelo', otroModeloInput.value); } else { otroModeloContainer.style.display =", "Jumpy", "Jumper"], "Fiat": ["500", "Panda", "Punto", "Tipo", "Toro", "Strada", "Argo", "Uno", "Ducato", "Fiorino"], "Jeep": ["Renegade", "Compass", "Cherokee", "Grand Cherokee", "Wrangler", "Gladiator", "Avenger", "Commander", "Wagoneer", "Patriot"], "Honda": ["Fit", "City", "Civic", "Accord", "CR-V", "HR-V", "Pilot", "BR-V", "Ridgeline", "Insight"], "BMW": ["Serie 1", "Serie 2", "Serie 3", "Serie 4", "Serie 5", "Serie 7", "X1", "X3", "X5", "Z4"], "Audi": ["A3", "A4", "A6", "A8", "Q2", "Q3", "Q5", "Q7", "Q8", "TT"], "Mercedes-Benz": ["A-Class", "C-Class", "E-Class", "S 'none'; otroModeloInput.required = false; updateLiveData('modelo', modeloSelect.value); } anioSelect.disabled = false; populateAnios(); updateImageButtonVisibility(); });
    ...
    if-Class", "GLA", "GLC", "GLE", "GLS", "CLA", "G-Class"], "Porsche": ["911", "Cayman", "Boxster", "Macan", "Cayenne", "Taycan", "Panamera", "718", "924", "928"] };
    const marcasOtras = Object.keys(marcasFullList).filter(m => !marcasPopulares.includes(m));
    const marcasOrdenadas = [...marcasPopulares, ...marcasOtras];
    const updateImageButtonVisibility = () => { if (!marcaInput || !modeloSelect || !anioSelect || !imageRefButton) return; const brand = marcaInput.value; const model = (modeloSelect.value === 'Otro(otroAnioInput) otroAnioInput.addEventListener('input', () => { updateLiveData('anio', otroAnioInput.value); updateImageButtonVisibility(); });
    ...
    if') ? otroModeloInput.value : modeloSelect.value; const year = (anioSelect.value === 'Otro') ? otroAnioInput.value : anioSelect.value; if (brand && model && year && brand !== 'Otro' && model !== '' && year !== '') { imageRefButton.classList.add('visible'); } else { imageRefButton.classList.remove('visible'); } };
    ...
    const checkAllFields = () => { if (!form || !sendPrompt) return; let allValid = true; form.(telefonoInput) telefonoInput.addEventListener('input', () => updateLiveData('telefono', telefonoInput.value));
    if(imageRefButton) imageRefButton.addEventListener('click', (e) => { e.preventDefault(); const brand = marcaInput.value; const model = (modeloSelect.value === 'Otro') ? otroModeloInputquerySelectorAll('[required]:not([type=hidden])').forEach(input => { let container = input.closest('.otro-input-container'); if (!container || container.style.display === 'block') { if (!input.value) allValid = false; } }); sendPrompt.style.display = allValid ? 'block' : 'none'; };
    const validateStep = (stepIndex) => { if (!formSteps || !formSteps[stepIndex]) return true; let errors =.value : modeloSelect.value; const year = (anioSelect.value === 'Otro') ? otroAnioInput.value : anioSelect.value; const cleanModel = model.replace(/\s*\(.*\)/g, '').trim(); const searchTerm = `${brand} ${cleanModel} ${year}`; const searchUrl = `https://www.google. []; formSteps[stepIndex].querySelectorAll('[required]').forEach(input => { let fieldLabel = document.querySelector(`label[for='${input.id}']`); if (input.offsetWidth > 0 || input.type === 'hidden') { const container = input.closest('.otro-input-container'); if (!container || container.style.com/search?tbm=isch&q=${encodeURIComponent(searchTerm)}`; const popupWidth = 800display === 'block') { if (!input.value) { errors.push(fieldLabel ? fieldLabel.textContent.replace(, popupHeight = 600; const left = (screen.width / 2) - (popupWidth / 2/:/g, '') : 'Campo requerido'); } } } }); if (errors.length > 0) {); const top = (screen.height / 2) - (popupHeight / 2); const popupFeatures = if (errorList && validationPopup) { errorList.innerHTML = ''; errors.forEach(err => { const li `width=${popupWidth},height=${popupHeight},top=${top},left=${left},resizable=yes,scroll = document.createElement('li'); li.textContent = `• ${err}`; errorList.appendChild(li); });bars=yes,status=yes`; window.open(searchUrl, 'imagePopup', popupFeatures); });
 validationPopup.classList.add('show'); clearTimeout(popupTimeout); popupTimeout = setTimeout(() => { validationPopup.classList.    if(form) form.addEventListener('submit', function(e) { e.preventDefault(); if (validateStep(currentStep)) { const formData = new FormData(form); let message = `*SOLICITUD DE REPUESTOremove('show'); }, 4000); } return false; } return true; };
    const populateAnios = () => { if (!anioSelect) return; anioSelect.innerHTML = '<option value="">Selecciona el año</option*\n\n`; message += `*VEHÍCULO:*\n`; message += `Marca: ${formData>'; for (let y = new Date().getFullYear() + 1; y >= 1990; y--) anioSelect.add(new Option(y, y)); anioSelect.add(new Option("Otro", ".get('marca') === 'Otro' ? formData.get('otra-marca') : formData.get('marca')}\n`; message += `Modelo: ${formData.get('modelo') === 'Otro' ? formData.get('otro-Otro")); };
    const updateLiveData = (field, value) => { const displayElement = document.getElementById(`display-${field}`); if (!displayElement) return; const span = displayElement.querySelector('span'); if (value) { span.modelo') : formData.get('modelo')}\n`; message += `Año: ${formData.get('anio')textContent = value; displayElement.style.display = 'block'; } else { displayElement.style.display = === 'Otro' ? formData.get('otro-anio') : formData.get('anio')}\n\n 'none'; } checkAllFields(); };
    const handleMarcaSelection = (marca, wrapper) => { if`; message += `*REPUESTO:*\n`; message += `Descripción: ${formData.get('descripcion') (!brandDisplay || !modeloSelect || !anioSelect || !otroMarcaContainer || !otroModeloContainer) return; brandDisplay.classList.add('visible'); const logoSrc = wrapper.querySelector('img')?.src || 'images/logos}\n`; message += `VIN: ${formData.get('vin') || 'No proporcionado'}\n\n`; message += `*CONTACTO:*\n`; message += `Nombre: ${formData.get('nombre')}\n`; message += `Teléfono: ${formData.get('telefono')}\n`; message += `Ubic/otra.png'; brandDisplayLogo.src = logoSrc; brandDisplayName.textContent = marca; modeloSelect.innerHTML = '<option value="">Selecciona un modelo</option>'; anioSelect.innerHTML = '<option value="">Primero selecciona un modelo</option>'; anioSelect.disabled = true; otroMarcaContainer.style.display = 'none'; otraación: ${formData.get('ubicacion') || 'No proporcionado'}\n`; const whatsappURL = `https://wa.me/593999115626?text=${encodeURIComponent(message)}`; window.open(whatsappURL, '_blank'); } });
    if (bgVideo) { const videos = ['MarcaInput.required = false; otroModeloContainer.style.display = 'none'; otroModeloInput.required = false; wrapper.style.setProperty('--logo-glow-url', `url(${logoSrc})`); brandDisplayLogoContainer.style.setProperty('--logo-glow-url-display', `url(${logoSrc})`); updateLiveData('modelo', ''); updateLiveData('images/videos/1.mp4', 'images/videos/2.mp4', 'images/videos/3.mp4', 'images/videos/4.mp4']; let currentVideoIndex = 0; bgVideo.playbackRate = 0.7; const playNextVideo = () => { currentVideoIndex = (currentVideoIndex + 1) % videos.length; const source = bgVideo.querySelector('source'); if(source) { sourceanio', ''); updateImageButtonVisibility(); if (marca === "Otro") { marcaInput.value = "Otro"; anioSelect.disabled = false; populateAnios(); modeloSelect.disabled = false; modeloSelect.innerHTML = '<option value="Otro" selected>Otro (Especifique)</option>'; otroMarcaContainer.style.display =.src = videos[currentVideoIndex]; bgVideo.load(); bgVideo.play().catch(error => console.log('Autoplay para el siguiente video fue prevenido:', error)); } }; bgVideo.addEventListener('ended', playNextVideo); document.body.addEventListener('click', () => { if (bgVideo.paused) { bg 'block'; otraMarcaInput.required = true; otroModeloContainer.style.display = 'block'; otroModeloInput.required = true; } else { marcaInput.value = marca; if (marcasFullList[marca]) { marcasFullList[marca].forEach(modelo => modeloSelect.add(new Option(modelo, modelo))); } modeloSelect.add(new Option("Otro", "Otro")); modeloSelect.disabled = false; } checkAllFields(); setTimeout(() => { if (modeloSelect) modeloSelect.scrollIntoView({ behavior: 'smooth', block: 'center' }); }, 100); };
    const populateLogos = () => {
        if (!logosContainer) {
            console.error("Error: El contenedor de logos no fue encontrado.");
            return;
        ...
        marcasOrdenadas.forEach(marca => {
            ...
            wrapper.className = 'logo-wrapper';
            const img = document.createElement('img');
            const span =Video.play().catch(error => console.log('Autoplay inicial fue prevenido:', error)); } }, { once: true }); }
